...
// Editor utility to validate EnemyDetailsSO assets and their prefabs
public static class EnemyPrefabValidator
{
    [MenuItem("Tools/Validate Enemy Prefabs")]
    public static void ValidateEnemyPrefabs()
    {
        string[] guids = AssetDatabase.FindAssets("t:EnemyDetailsSO");
        int errors = 0;
        foreach (var g in guids)
        {
            string path = AssetDatabase.GUIDToAssetPath(g);
            var asset = AssetDatabase.LoadAssetAtPath<EnemyDetailsSO>(path);
            if (asset == null) continue;
            if (asset.enemyPrefab == null)
            {
                Debug.LogWarning($"EnemyDetailsSO '{asset.name}' has no enemyPrefab assigned ({path}).");
                errors++;
                continue;
            }
        ...
        if (errors == 0)
            EditorUtility.DisplayDialog("Enemy Prefab Validation", "No issues found for EnemyDetailsSO prefabs.", "OK");
        else
